...
class TriggerEvent(private val event : Event) : Event(
    card = event.card,
    ...
    lateinit var scope: Card

    override fun hasGainType(t: GainType): Boolean {
        return event.hasGainType(t)
    }

    suspend inline fun <reified T> let(action : suspend (Card) -> T) : T{
        return action(scope)
    }

    suspend fun test(action : suspend (Card) -> Boolean) : Boolean {
        return action(scope)
    }
    ...
    fun updateScope(next : Card) { scope = next }
