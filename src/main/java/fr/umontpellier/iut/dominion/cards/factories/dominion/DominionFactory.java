...
//                .setup(config -> config
//                        .onPlay(run((Player player, Card self) -> CardUtil.gainFromSupply(player, "Choisi une carte coûtant au maximum 5", c -> c.isAtMost(5), Destination.HAND, false))
//                                .then((player, self) ->
...
//                                        CardUtil.gainFromSupply(player, "Gold", Destination.DISCARD, false)).then((player, self) ->
...
//                .setup(config -> config
//                        .onPlay(run((Player player, Card self) -> CardUtil.gainFromSupply(player, "Silver", Destination.DRAW, false))
//                                .then((player, self) ->
...
//                                owner.log(String.format("Trigger %s : %s récupère une pièce", config.get().getName().toUpperCase(), c.player.getName()));
//                                config.get().set("used", true);
//                        })
//                        .cardPlayedCondition((event, player) -> (!config.get().getFlag("used") && event.getCard().hasName("Silver") && event.player == player))
//                );
//    }
//    @Dominion_Card(extension = "Dominion")
...
//                                                        player,
//                                                        "Choose a Treasure (Max " + (card.getCostValue()+3) + "$)",
//                                                        filter -> filter.hasType(CardType.TREASURE) && filter.isAtMostWithBonus(card, 3) ,
//                                                        Destination.HAND,
//                                                        false),
//                                                gained -> {
//                                                    player.log(String.format("Action %s : %s trash %s for %s", self.getName().toUpperCase(), player.getName(), card.getName().toUpperCase(), gained.getName().toUpperCase()));
//                                                    return null;
//                                                }
//                                        );}
//                                    }
...
//                                        CardUtil.gainFromSupply(
//                                                player,
//                                                "Choisi une carte (Max " + (card.getCostValue()+2) + " pièces)",
...
//                                    "y"::equals,
//                                    choice -> {
//                                        player.getCopyOf(Destination.DRAW).forEach(card -> player.moveTo(card, Destination.DISCARD));
//                                        return null;
//                                    },
//                                    () -> null
//                            );
...
//                                            "y"::equals,
//                                            choice -> {
//                                                gain(player, Final, Destination.DISCARD);
//                                                return null;
//                                            },
//                                            () -> null
//                                    );
