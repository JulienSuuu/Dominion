...
//    public static final Predicate<Card> activate = card -> card.getComponent<DurationComponent>().map(d -> !d.isFinished(card) && card.hasForLocation(Destination.INPLAY)).orElse(false);
...
//        boolean exceeds = selection.getCostValue() > purchased.getCostValue() ||
//                selection.getPotion() > purchased.getPotion() ||
//                selection.getDebt() > purchased.getDebt();
//
//        if (exceeds) return false;
//
//        return selection.getCostValue() < purchased.getCostValue() ||
//                selection.getPotion() < purchased.getPotion() ||
//                selection.getDebt() < purchased.getDebt();
//    };
//
...
//           return isAtTavernMat && condition.test(event, player) && event.player == player;
//       });
//    }
//
